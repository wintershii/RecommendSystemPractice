...
class NaiveBayesian:
    ...
    def train(self, data, labels):
        ...
        # 是异常用户的概率
        self.classP[1] = ((sum(labels) + self.alpha) * 1.0 /
                          (numData + self.alpha * len(set(labels))))
        # 不是异常用户的概率
        self.classP[0] = 1 - self.classP[1]

        # 用来存放每个label下每个特征标签下对应的高斯分布的均值和方差
        self.classP_feature = dict()
        for c in set(labels):
    ...
    def gaussian(self, mu, sigma, x):
        return 1.0 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-(x - mu)**2 /
                                                           (2 * sigma**2))
    ...
    def predict(self, x):
        ...
        for key in self.classP.keys():
            ...
            for fp in feature_p.keys():
                currentP *= self.gaussian(feature_p[fp][0], feature_p[fp][1],
                                          x[j])
                j += 1
